...
class Api1 extends CI_Controller {
                ...
                if(strtotime($scheduleDate)>strtotime(date('Y-m-d'))){
                    ...
                    if ($itemResult) {
                        ...
                        $itemArray[] = array(
                            ...
                            "delivery"           => $key['quanity'] * $itemResult->delivery,
                            ...
                            // if ($dlt) {
                                ...
                                if ($getUserId) {
                                    ...
                                    $dataNotification =array(
                                        ...
                                        "message"   => "Your order have cancelled  Successfully!!( Order Id $order_id ) ",
                                        "read_status"=> '0'
                                    );
                                    $createNotification = $this->Model->insert('notification',$dataNotification);
                                }
                                if ($results_user_data) {
                                    if($results_user_data->device_type=='android'){
                                        $a= $this->sendPushAndroid($results_user_data->device_token,'Message Alert','Your Order Cancelled Successfully!!',$user_id,'0');
                                    }
                                    $data_result['result']     = true;
                                    $data_result['msg']     = 'Order Deliver';
                                }else{
                    ...
                    foreach ($results as $key) {
                        ...
                        if($chckloop=="true"){
                            ...
                            // if ($dlt) {
                                ...
                                $msg="Your order has been Return  Successfully!!( Cart Id:".$cart_id.")";
                                if ($getUserId) {
                                    $user_id = $getUserId->user_id;
    ...
    public function orderHistoryBitweenDate(){
        ...
        //$results    = $this->Model->selectdata('cart',$where);
        $results = $this->db->query("SELECT * FROM `cart` WHERE `user_id` = '".$user_id."' AND `cart_status` = '1' AND date_time >= '".$from_date."' AND date_time <= '".$to_date."' ORDER BY date_time ASC")->result_array();
        if ($results) {
            foreach ($results as $key){
                $item_id = $key['item_id'];
                $whereData1 = array(
                ...
                if($res){
                        ...
                        if($results_cart){
                            ...
                            $dataNotification =array(
                                ...
                                "date_time" => $date,
                                "title"     => "Order Deliver",
                                "message"   => "Your Order Deliver Successfully!!",
                                "read_status"=> '0'
                            );
                            $createNotification = $this->Model->insert('notification',$dataNotification);
                            if ($results_user_data) {
                                if($results_user_data->device_type=='android'){
                                    $a= $this->sendPushAndroid($results_user_data->device_token,'Message Alert','Your Order Deliver Successfully!!',$user_id,'0');
                                }
                                $data_result['result']     = true;
                                $data_result['msg']     = 'Order Deliver';
                            }else{
                    ...
                    $results_cart    = $this->Model->selectAllById('cart',$where_device);
                        if($results_cart){
                            ...
                            if ($results_user_data) {
                                if($results_user_data->device_type=='android'){
                                    $a= $this->sendPushAndroid($results_user_data->device_token,'Message Alert','Your Order Return Successfully!!',$user_id,'0');
                                }
                                $data_result['result']     = true;
...
WHERE cart_status = '4' AND date_time BETWEEN (CURDATE() - INTERVAL 28 DAY) AND CURDATE() ORDER BY cart_id DESC")->result_array();
    ...
    public function Search()
    {
        ...
        $results = $this->db->query("SELECT i.*,s.sub_cat_name FROM item as i JOIN sub_cat as s ON s.sub_cat_id = i.sub_cat_id  WHERE item_name like '%".$search."%' AND is_online = 'yes' ")->result_array();
        if ($results) {
            foreach ($results as $key) {
                $res[] = array( 
                   'item_id'          => $key['item_id'],
